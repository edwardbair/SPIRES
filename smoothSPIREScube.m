...
%
%2023-10-26, Jeff modified method to identify the peak grain size and dust
%to fix

%0.75-1.5 hrfor Sierra Nevada with 60 cores
time1=tic;

h5name=fullfile(outloc,[nameprefix datestr(matdates(end),'yyyy') '.h5']); %#ok<*DATST>
...
elseif exist(lockname,'file')==2
    ...
    for i=1:size(dv,1)
        ...
        doy_start=datenum(dv(i,:))-datenum(dv(1,:))+1; %#ok<*DATNM>
    ...
    out.grainradius(out.fsca==0) = NaN; % Jeff added

    %find max grain size (perhaps not used given Jeff's mods)
    ...
    idx=max(idx,[],3); %#ok<NASGU>
    %Nd=15; %N days to taper
    NedMethod = false; % hack here, flag to keep Ned's code
    if fixpeak % set values after peak grain radius to peak
        if NedMethod
            for i=1:size(idx,1) %#ok<UNRCH>
                for j=1:size(idx,2)
            ...
            for i=1:size(out.grainradius,1)
                for j=1:size(out.grainradius,2)
                    %get rid of spikes & drops
                    rgvec=squeeze(out.grainradius(i,j,:));
                    rgvec=hampel(rgvec,2,2);
                    % fSCA vector
                    fSCA = squeeze(out.fsca(i,j,:));
                    % peaks
                    [~,glocs] = findpeaks(rgvec);
                    [~,flocs] = findpeaks(fSCA);
                    % if grain size peaks after fSCA peaks, keep sizes at
                    % or above that value till the end
                    % elseif fSCA peaks after grain size peaks, keep the grain
                    % size of the fSCA peak as the minimum till the end
                    rgholdNaN = isnan(rgvec); % hold onto the NaNs as they don't change
                    endDay = max(glocs(end),flocs(end));
                    t = false(size(rgvec));
                    tt = rgvec<rgvec(endDay);
                    t(endDay:end) = tt(endDay:end);
                    rgvec(t) = rgvec(endDay);
                    rgvec(rgholdNaN) = NaN;
                    out.grainradius(i,j,:)=rgvec;
                    ...
                    %all days prior to max grain size
                    ttstart=false(size(tt));
                    maxDay = find(rgvec==max(rgvec));
                    ttstart(1:maxDay(1))=true;
                    %all days prior to max grain size with small grains
                    tt=ttstart & tt;
                    %set dust on those days to zero
                    dustvec(tt)=0;
                    % hang onto the zero values
                    dustZero = dustvec==0;

                    %use dust value from max grain size day
                    dval=out.dust(i,j,endDay);
                    %set dust after those days to value on endDay
                    dustvec(endDay:end)=dval;
                    % reset zeros
                    dustvec(dustZero) = 0;
                    out.dust(i,j,:) = dustvec;
                end
