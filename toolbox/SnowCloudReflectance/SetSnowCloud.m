...
%   'muS' - cosine of illumination angle on slope, scalar or same size as cosZ
%   'cosS' - cosine of slope, same size as muS
...
%   For fractional snow, reflectance can have one or more columns depending
%   on whether pixel has one or more non-snow endmembers
...
%   that sums to 1.0 (Mem1 and Mem2 might be soil and vegetation, for example,
%   or there might be a shade endmember, or multiple soils, etc)
% alternatively, 'fSCA' can be specified instead, in which case there is
%   one non-snow endmember and fractionalCoverage is set to [fSCA 1-fSCA]
%%
...
newWavelength = ~issens && ~isbandp && iswave;
newSensor = ~iswave && ~isbandp && issens;
newBandPass = ~iswave && ~issens && isbandp;
if newWavelength
    fscript.sensor = [];
    fscript.bands = [];
    fscript.bandPass = [];
