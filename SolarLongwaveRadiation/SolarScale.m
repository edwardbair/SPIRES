...
if doSmooth || isempty(p.Results.wavelength)
    ...
    [wavelength,cosZ,elevation] = ndgrid(p.Results.wavelength,cosZ,elevation);
end
...
if ~isempty(p.Results.wavelength) && doSmooth
    % knot at every wavelength input wavelength and at the ends
    uWave = unique(p.Results.wavelength(:));
    waveKnots = [min(Tbl.wavelength); uWave; max(Tbl.wavelength)];
    P = slmset('knots',waveKnots);
    if doTOA
        slm = slmengine(Tbl.wavelength,Tbl.TOA,P);
        newTbl = table(uWave,slmeval(uWave,slm),'VariableNames',...
            {'wavelength','TOA'});
        ...
        for k=1:length(ucos)
            ...
            thisTbl = table(uWave,repmat(ucos(k),length(uWave),1),...
                repmat(uelev(k),length(uWave),1),slmeval(uWave,slm1),...
                slmeval(uWave,slm2),'VariableNames',...
                {'wavelength','cosZ','elevation','Global','DiffuseFraction'});
