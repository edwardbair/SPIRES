function out=smoothSPIREScube(nameprefix,outloc,matdates,...
    ...
    fixpeak,max_el_drylakes,scdf_drylakes)
...
% b_R - b/R ratio for canopy cover, see GOvgf.m, e.g. 2.16
...
% max_el_drylakes - max dry lake elevation, m, e.g. 1600
% scdf_drylakes - min snow cover days for dry lakes, fraction, e.g. 0.7
% together w/ el_drylakes (and cc==0) these are used to set dry lakes to
% NaN
...
if exist(h5name,'file')
    ...
    %dry lakes (and bright soils) mask
    %  low elevation, no canopy cover, high snow cover days percentage

    drylakesMask = (Z < max_el_drylakes) & (cc == 0) & ...
        ((sum(out.fsca > fsca_thresh,3)./sum(out.fsca >= 0,3)) > ...    
        scdf_drylakes);
    
    mask=mask | drylakesMask;
    %masked area filter
