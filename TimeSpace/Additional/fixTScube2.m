function output_file = fixTScube2(folder,prefix,tile,WY)
% output_file = fixTScube2(folder,prefix,tile,WY)
% fix the TS cubes to make sure they contain the whole year
% (also need to fix consolidateTimeSpace to do this)
%
% Input
%   folder - where all the current file lives (if on the
...
for k=1:length(variable)
    ...
    parfor c=1:size(X,2)/24000
        if ~all(isnan(X(:,c)))
            ...
            t = ~isnan(y);
            F = fit(x(t),y(t),'linearinterp'); %#ok<PFBNS>
            Z(:,c) = F(allDays);
        end
    end
...
if isfield(D.(member),'FillValue') 
    if D.(member).FillValue~=0 && min(D.(member)(:))~=D.(member).FillValue
